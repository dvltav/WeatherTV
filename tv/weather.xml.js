var Template = function(temp) { return `<?xml version="1.0" encoding="UTF-8" ?>
...
<document>
   ...
   <background>
      ...
      <banner>
         <title ><text color='black'>Outside ${temp}, ${new Date().getHours() + ":" + new Date().getMinutes()}</text></title>
<!--
         <row>
...
-->
      ...
      <carousel>
         <section>
            ...
            <lockup action='showDetail' dest="http://pixelcaster.com/yosemite/webcams/ahwahnee2.jpg?${new Date().getTime()}">
               <img src="http://pixelcaster.com/yosemite/webcams/ahwahnee2.jpg?${new Date().getTime()}" width="1200" height="800" />
               <title>Yosemite</title>
            </lockup>
            <lockup action='showDetail' dest="http://192.168.1.140/weewx/daytempdew.png?${new Date().getTime()}">
               <img src="http://192.168.1.140/weewx/daytempdew.png?${new Date().getTime()}" width="800" height="600" />
               <title>Temperature</title>
            </lockup>
            <lockup action='showDetail' dest="http://192.168.1.140/weewx/daywind.png?${new Date().getTime()}">
              <img src="http://192.168.1.140/weewx/daywind.png?${new Date().getTime()}" width="800" height="600" />
              <title>Wind</title>
            </lockup>
            <lockup action='showDetail' dest="http://192.168.1.140/weewx/weektempdew.png?${new Date().getTime()}">
            ...
            <lockup action='showDetail' dest="http://192.168.1.140/weewx/dayrain.png?${new Date().getTime()}">
               <img src="http://192.168.1.140/weewx/dayrain.png?${new Date().getTime()}" width="800" height="600" />
               <title>Rain</title>
            </lockup>
            <lockup action='showDetail' dest="http://radar.weather.gov/ridge/lite/N0R/MUX_0.png" >
               <img src="http://radar.weather.gov/ridge/lite/N0R/MUX_0.png?${new Date().getTime()}" width="800" height="600" />
               <title>Radar</title>
            </lockup>
         </section>
      </carousel>
